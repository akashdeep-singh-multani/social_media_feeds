...
export class PostCommentListComponent {
  ...
  constructor(
    ...
    private store: Store<{ comments: { comments: Comment[] } }>,
    public dialogRef: MatDialogRef<PostCommentListComponent>,
  ...
  ngOnInit() {
    ...
    this.socketManagerService.newPostCommentReceived$.subscribe(
      (comment: any) => {
        this.handleNewComment(comment);
      }
    );
    this.socketManagerService.notificationReceived$.subscribe(
      (notification: any) => {
